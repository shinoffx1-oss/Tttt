...
async def like_player():
    try:
        ...
        requested_region = (request.args.get("region") or "").upper().strip()
        if requested_region:
            if requested_region not in _SERVERS:
                return jsonify({
                    "error": "Invalid region",
                    "message": "Region must be BR, IND or EUROPE",
                    "status": 400,
                    "credits": "https://t.me/nopethug"
                }), 400
            tokens = _token_cache.get_tokens(requested_region)
            if not tokens:
                return jsonify({
                    "error": "Region unavailable",
                    "message": f"No tokens available for {requested_region}",
                    "status": 503,
                    "credits": "https://t.me/nopethug"
                }), 503
            info_url = f"{_SERVERS[requested_region]}/GetPlayerPersonalShow"
            response = await async_post_request(info_url, bytes.fromhex(encode_uid(uid)), tokens[0])
            player_info = decode_info(response) if response else None
